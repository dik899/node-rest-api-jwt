    ...
    next(err);
      ...
      let movieList = [];
          if(err)
            ...
            res.json({status:"success", message: "Movie deleted successfully!!!", data:{movies : movieList}});
            }
        });
      }
  });
 ...
 create: function(req, res, next) {
  movieModel.create({ name: req.body.name, released_on: req.body.released_on }, function (err, result) {
      if (err) 
       ...
       res.json({status: "success", message: "Movie added successfully!!!", data:{name: req.body.name, released_on : req.body.released_on}});
